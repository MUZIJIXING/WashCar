...
class EfficientNetPredictor:
    def __init__(self, model_path, num_classes=10):
        ...
        self.net = EfficientNet.from_name('efficientnet-b0', num_classes=self.num_classes)
        if self.CUDA:
            self.net.cuda()
...
def classify_image(image):
    model_path = '../total_modal/EfficientNet_best_model.mod'  # 使用最佳模型文件
    predictor = EfficientNetPredictor(model_path)
